...
import CityCombobox from '../components/CityInput';
...
export default function CreateRide() {
  ...
  const [driverId, setDriverId] = useState('drv-seed'); // PoC
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const navigate = useNavigate();

  // Convertit yyyy-mm-dd + HH:mm en ISO (UTC Z)
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirection simple vers la recherche préremplie
      navigate(
        `/results?from=${encodeURIComponent(originCity)}&to=${encodeURIComponent(destinationCity)}`
      );
    } catch (e: any) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] bg-gradient-to-b from-zinc-900 to-black text-zinc-100">
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        >
          {/* Départ / Arrivée avec autocomplete */}
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-zinc-400 mb-2">Départ</label>
              <CityCombobox
                id="create-from"
                label="Départ"
            ...
            <div>
              <label className="block text-sm text-zinc-400 mb-2">Arrivée</label>
              <CityCombobox
                id="create-to"
                label="Arrivée"
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl bg-zinc-800/80 border border-zinc-700 px-4 py-3 outline-none focus:ring-2 ring-indigo-500 [color-scheme:dark]"
                required
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-zinc-500">
              * Au clic, on crée le trajet côté <code>ride</code> puis on te redirige vers la recherche.
            </span>
          </div>
        </motion.form>
      </div>
