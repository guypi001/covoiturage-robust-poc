...
export class EventBus implements OnModuleInit, OnModuleDestroy {
  ...
  private async ensureProducer(retry = 3) {
    ...
    while (retry-- > 0) {
      try { await this.producer.connect(); this.connected = true; return; }
      catch (e:any) { console.error('[EventBus] producer connect failed, retrying:', e?.message || e); await new Promise(r => setTimeout(r, 800)); }
    }
  }
  async publish(topic: string, message: any, key?: string) {
    ...
    } catch (e:any) { console.error('[EventBus] publish failed:', topic, e?.message || e); }
  }
  async subscribe(groupId: string, topic: string, handler: (msg:any)=>Promise<void>|void) {
    ...
    await consumer.run({
      eachMessage: async ({ message }) => {
        ...
        try { await handler(JSON.parse(message.value.toString('utf-8'))); }
        catch (e) { console.error('Event handler error for topic', topic, e); }
      }
    });
